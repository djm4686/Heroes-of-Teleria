...
class MoveAnimation(heroanimation.HeroAnimation):
    def __init__(self, startTile, endTile, hero):
        ...
        self.destTile = None
        self.hero = hero
        self.hpBar = hpbar.HPBar(self.hero.maxhp, self.hero.currentHp, self.x, self.y)
        self.hpBar.setCenter((self.x, self.y))
        self.endTile = endTile
        self.shadow = pygame.image.load("images/shadow.png")
        self.shadowRect = pygame.Rect(self.x, self.y, self.shadow.get_rect().width, self.shadow.get_rect().height)
    ...
    def animate(self, surface):
        ...
        if self.nextNode != None and self.hero.getTile() == None:
            ...
            if self.hero.getTile != None:
                ...
                self.shadowRect.center = (self.x, self.y)
                surface.blit(self.shadow, self.shadowRect)
                self.hpBar.setPoint(self.x, self.y)
                self.hpBar.rect.x -= 25
                self.hpBar.hpRect.x -= 25
                self.hpBar.rect.y -= 60
                self.hpBar.hpRect.y -= 60
                p = pygame.Rect(self.x, self.y, 60, 60)
                p.center = (self.x, self.y - 25)
