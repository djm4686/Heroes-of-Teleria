...
class Hero:
    def __init__(self, ID, name, heroclass, race, playerID = 0):
        ...
        self.selectedImage = pygame.image.load("images/medium_blue_particle.png")
        self.originalImage = pygame.image.load("images/medium_blue_particle.png")
        self.constitution = 5
        ...
        self.hpBar = hpbar.HPBar(self.maxhp, self.currentHp, 0, 0)
        self.i = 0
        self.updateModifyers()
    def updateModifyers(self):
        ...
        for x in self.spellEffects:
            ...
            elif x.isHitChanceMod():
                self.hitChanceMod *= x.getAmt()
    ...
    def getMovementRange(self):
        return self.range + 1
    def hitChance(self, attacker, defender):
        baseChance = 100
        if attacker.direction == defender.direction:
            baseChance = 90
        elif abs(attacker.direction - defender.direction) == 2:
            baseChance = 60
        elif abs(attacker.direction - defender.direction) == 1 or ((attacker.direction == 4 and defender.direction == 1) or (attacker.direction == 1 and defender.direction == 4)):
            baseChance = 75
        return baseChance * self.hitChanceMod
    def meleeAttack(self, hero):
        if random.randrange(0, 100) > self.hitChance(self, hero):
            return None
    ...
    def draw(self, surface, moving=False):
        ...
        if not self.isDead():
            ...
            r.center = self.tile.getCenter()
            r.y = r.y - 25
            shadowRect = pygame.Rect(self.tile.getCenter(), (30,15))
            shadowRect.center = self.tile.getCenter()
            surface.blit(self.shadow, shadowRect)
            try:
                if self.selected:
                    self.selectedImage = pygame.transform.smoothscale(self.originalImage, (r.width, r.height))
                    surface.blit(self.selectedImage, r)
            ...
            r.x -= 8
            r.y = r.y - 55
            self.hpBar.setPoint(r.x, r.y)
            self.hpBar.update(self.currentHp)
