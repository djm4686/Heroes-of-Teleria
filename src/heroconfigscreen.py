...
class HeroConfigScreen:
    ...
    def makeMeleeUI(self):
        if len(self.weaponButtons) == 0:
            for w in MELEE_WEAPONS:
                    self.weaponButtons.append(button.Button(pygame.font.Font(pygame.font.get_default_font(), 12).render(w.getName(), True, (0,0,0)), self.assignMeleeWeapon, [w]))
        else:
            self.weaponButtons = []
    ...
    def makeStatsComponent(self, surface):
        self.rangedButton = multilinetextbutton.MultiLineTextButton([self.makeText("Ranged"), self.makeText("Equipped:"), self.makeText(self.hero.getRangedWeapon().getName())], self.makeRangedUI, rect = pygame.Rect(600, 150, 200, 100))
        self.armorButton = multilinetextbutton.MultiLineTextButton([self.makeText("Armor"), self.makeText("Equipped: "), self.makeText(self.hero.getArmor().getName())], self.makeArmorUI, rect = pygame.Rect(600, 250, 200, 100))
        self.shieldButton = multilinetextbutton.MultiLineTextButton([self.makeText("Shield"), self.makeText("Equipped:"), self.makeText(self.hero.getShield().getName())], self.makeShieldUI, rect = pygame.Rect(600, 350, 200, 100))
        self.meleeButton = multilinetextbutton.MultiLineTextButton([self.makeText("Melee"), self.makeText("Equipped:"), self.makeText(self.hero.getMeleeWeapon().getName())], self.makeMeleeUI, rect = pygame.Rect(600, 50, 200, 100))
        self.heroClassLabel = pygame.font.Font(pygame.font.get_default_font(), 34).render(self.hero.getActiveHeroClass().getName(), True, (0,0,0))
